...
def comecar():
    global indice_atual, btn_play, btn_quit
    indice_atual = 0
    btn_play.destroy()
    btn_quit.destroy()
...
def AbrirResultados():
    global bg_menu, btn_play, btn_quit
    bg_menu = canvas.create_image(0, 0, anchor=NW, image=bg)
    ...
    btn_play = Button(root,text="REPLAY",compound=CENTER,width=6, height=2,border=False,bg="#FAEA0C",command=lambda:comecar())
    btn_play.place(x=236, y=780)
